...
interface StoryStore {
  ...
  // 全局状态
  status: 'idle' | 'generating_prompts' | 'generating_images' | 'completed' | 'error'
...
scenes.forEach((scene, index) => {
  generateImage(scene.img_text_prompt)
    .then(url => updateScene(index, { imageUrl: url, imageStatus: 'success' }))
    .catch(err => updateScene(index, { imageStatus: 'error', error: err.message }))
})
```
...
interface StoryStore {
  ...
  // 操作 - 场景数据
  initScenes: (scenes: Array<{ text: string; img_text_prompt: string }>) => void
...
const handleSubmit = async (formData: FormData) => {
  const { setFormData, initScenes, setStatus, setError } = useStoryStore.getState()
  ...
  return (
    <div className="border rounded-lg p-4">
      ...
      {scene.imageStatus === 'success' && scene.imageUrl && (
        <img src={scene.imageUrl} alt={scene.text} className="h-40 object-cover" />
      )}
