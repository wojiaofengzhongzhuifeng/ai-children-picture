...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!illustration_style) {
      ...
      errors.push(
        `illustration_style 必须是以下值之一: ${VALID_STYLES.join(', ')}`
      );
    }
    ...
    const styleInfo =
      stylePromptMap[illustration_style] || stylePromptMap.watercolor;
...
[
  {
    ...
    try {
      ...
      for (let i = 0; i < scenes.length; i++) {
        ...
        if (
          !scene.img_text_prompt ||
          typeof scene.img_text_prompt !== 'string'
        ) {
