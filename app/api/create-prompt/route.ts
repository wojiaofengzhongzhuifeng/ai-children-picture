...
const VALID_THEMES = ['emotional_education', 'cognitive_learning', 'social_behavior', 'natural_science', 'fantasy_adventure']
  ...
  try {
    ...
    if (!themes || !Array.isArray(themes) || themes.length === 0) {
      ...
      if (invalidThemes.length > 0) {
        errors.push(`无效的主题: ${invalidThemes.join(', ')}。有效值: ${VALID_THEMES.join(', ')}`)
      }
    }
    ...
    const stylePromptMap: Record<string, { style: string; technique: string }> = {
      watercolor: {
        ...
        technique: 'delicate brush strokes, flowing colors, subtle color blending, transparent washes, soft edges'
      },
      crayon: {
        style: 'children crayon drawing style',
        technique: 'bold colorful strokes, textured lines, vibrant colors, hand-drawn feel, playful scribbles'
      },
      cartoon: {
        style: 'cute cartoon animation style',
        technique: 'clean outlines, bright flat colors, expressive characters, smooth gradients, Disney-Pixar inspired'
      },
      clay_3d: {
        style: '3D clay sculpture style, claymation',
        technique: 'soft rounded shapes, matte textures, sculpted details, stop-motion aesthetic, tactile appearance'
      },
      paper_cut: {
        style: 'paper cut collage art style',
        technique: 'layered paper textures, geometric shapes, torn edges, handcrafted feel, dimensional layers'
      }
    }
  ...
  return NextResponse.json({
      ...
      scenes: [
        {
          ...
          img_text_prompt: '用于AI生成插画的详细英文描述（包含角色外观、动作、场景、风格等）'
        }
      ],
      sceneCount: 'number',
    ...
    fieldExplanation: {
      ...
      img_text_prompt: {
        ...
        includes: ['角色外观特征', '动作姿态', '场景环境', '情绪氛围', '色彩基调', '插画风格', '构图视角'],
        example: 'A cute white rabbit with big round eyes and long floppy ears, wearing a small blue vest, sitting alone under a big oak tree in a meadow full of colorful wildflowers. The rabbit looks shy and hesitant, watching other animals playing in the distance. Soft watercolor style, warm pastel colors, gentle sunlight filtering through leaves, children\'s book illustration, high quality.'
      }
    },
    example: {
      request: {
