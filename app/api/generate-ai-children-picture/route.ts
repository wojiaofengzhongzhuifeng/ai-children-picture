  ...
  try {
    ...
    if (process.env.USE_MOCK_DATA?.toLowerCase().trim() === 'true') {
      ...
      return NextResponse.json(
        getMockResponse(prompt, negativePrompt, size, sceneIndex)
      );
    }
    ...
    // 8. 提取智谱返回的临时图片 URL
    const zhipuImageUrl =
      data.data?.[0]?.url || data.image_url || data.image_urls?.[0];
      ...
      return NextResponse.json(
        {
          error: '下载生成的图片失败',
          details:
            downloadError instanceof Error ? downloadError.message : '未知错误',
        },
        { status: 500 }
      );
    }
