import { ITextGenerator, TextGenerationParams, TextGenerationResult } from '@/lib/services/ai'
...
export class ZhipuTextGenerator implements ITextGenerator {
  ...
  async generate(params: TextGenerationParams): Promise<TextGenerationResult> {
    ...
    const requestBody = {
      ...
      messages: [
        ...(params.context ? [{ role: 'system' as const, content: params.context }] : []),
    ...
    console.log('正在调用 Zhipu 文本生成 API:', {
      ...
      prompt: requestBody.messages[requestBody.messages.length - 1]?.content?.substring(0, 50) + '...',
